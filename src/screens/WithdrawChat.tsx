...
const ChatScreen = () => {
  ...
  // Debug logging
  useEffect(() => {
    console.log("withdraw messages total count:", withdraw_message?.length);
  }, [withdraw_message]);

  // Set messages to local state to ensure we control rendering
  useEffect(() => {
    if (withdraw_message && withdraw_message.length > 0) {
      setMessagesData([...withdraw_message]);
    }
  }, [withdraw_message]);
  ...
  useEffect(() => {
    if (mobile) {
      selectUserChat(mobile, "withdraw");
    }
  }, [mobile]);
  ...
  const debugImageUrl = (url:any, messageId:any) => {
    ...
    fetch(url, { method: 'HEAD' })
      .then(response => {
        console.log(`Image URL status for message ${messageId}:`, response.status);
        console.log(`Content-Type for message ${messageId}:`, response.headers.get('content-type'));
      })
      .catch(error => console.error(`Error checking image URL for message ${messageId}:`, error));
  };
  ...
  const renderFileContent = (message:any) => {
    ...
    if (message?.file && typeof message.file === 'string') {
      // Debug the image URL if it's an HTTP URL
      if (message.file.startsWith('http')) {
        debugImageUrl(message.file, messageId);
      }
      ...
      if ((isDataUri && message.file.startsWith('data:image')) || 
          (isHttpUrl && /\.(jpg|jpeg|png|gif|webp)$/i.test(message.file))) {
  ...
  return (
    ...
    >
      <HeaderThree title='Withdraw Chat'/>
